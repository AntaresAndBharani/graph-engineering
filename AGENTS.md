...
1. **Single Communication Medium:** All exchanges happen strictly via the plan file (`--plan <path>`, default `docs/draft-requisites/implementation-plan.md`), passed to the helper script as `--plan`. Never use temporary buffers. Completed plans are archived to an `archive/` folder next to the plan file so the live file only holds the active plan.
