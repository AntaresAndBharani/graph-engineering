...
|---|---|---|---|---|---|
| **Node 1** | **Architect** (`architect.py`) | **Active / Enabled** (Producer) | Living architecture sync (7-day SLA), INVEST story decomposition, subtask linking, and PR architectural reviews. | Label `needs-triage` or Weekly SLA trigger | Pluggable Research Harness (`antigravity`/`gemini-3.8-flash-high`) / Primary (`claude-sonnet-5`) |
| **Node 2** | **3-Amigos DevTest** (`devtest.py`) | **Active / Enabled** (Consumer) | Pre-flight git safety check, deterministic lowest-ID task pickup, TDD test suite generation, clean implementation, autonomous PR opening, and auto-merge. | Label `ready-for-dev` or active story `queued` | Primary Implementation Harness (`antigravity` / `claude`) |
| **Node 0** | **Supervisor** (`supervisor.py`) | **Optional / Disabled** | Consistency watchdog, 12h SLA audit, proactive PO-proxy requirement evaluation, SHA-256 hash gating, and anomaly self-healing. | Scheduled interval (default 1h) or Label `needs-po-review` | Fast PO Evaluation (`antigravity`/`gemini-3.8-flash-high`) + Zero-token audit |
...
| **Configuration & Validation** | Pydantic v2 | `pydantic>=2.6.0` | Rust-backed schema validation, cross-platform path resolution (`~`, `$HOME`, `%USERPROFILE%`), strict runtime validation, and hot-reload model rebuilding (`OrchestratorBaseModel`). |
| **Persistence & State Engine** | aiosqlite (SQLite WAL) | `aiosqlite>=0.20.0` | Asynchronous file-based persistence, Write-Ahead Logging (WAL) concurrency, deterministic TTL distributed locking, idempotent migrations, and Artifact Blackboard store. |
...
| **Dynamic Runtime Reloading** | importlib + SQLite IPC | Standard Library | Deterministic hot reloading of configuration and in-memory Python modules without stopping the running daemon via centralized single-consumer watcher. |
...
| **Testing & Quality Assurance** | pytest + pytest-asyncio | `pytest>=8.0.0`, `pytest-asyncio>=0.23.0` | Asynchronous unit, mock, BDD, and integration test coverage with zero-token assertions. |
...
1. **Domain Core Layer (`orchestrator/config.py`, `orchestrator/logging.py`, `TokenUsageReader` protocol)**:
   - Holds core immutable entities, taxonomy schemas (`managed_labels`), harness definitions (`HarnessConfig`), quota configuration structures (`HarnessQuotaConfig`, `WindowLimitConfig`, `QuotaSettings`), and configuration data structures (`GlobalConfig`, `ProjectConfig`, `NodeConfig`, `SettingsConfig`).
...
2. **Infrastructure & Outbound Adapters Layer (`orchestrator/db.py`, `orchestrator/harness.py`, `orchestrator/quota.py`, `orchestrator/poller.py`, `orchestrator/housekeeping.py`, `orchestrator/reloader.py`, `orchestrator/worktree.py`)**:
   - Manages state persistence, idempotent migrations, distributed locking, and Blackboard artifacts via SQLite WAL transactions (`StateManager`).
   ...
   - Manages creation, synchronization, safe removal, stash protection (`git stash push -u`), and pruning of ephemeral git worktrees per node and project with serial execution fallback (`WorktreeManager`).
...
3. **Application Pipeline Nodes & Use Cases Layer (`orchestrator/nodes/*`, `orchestrator/cli.py:lifecycle`)**:
   - Houses the discrete workflow engines representing each stage of the engineering lifecycle:
     - **Default Active 2-Node Parallel Engine**:
       - `node-architect` (Node 1: Producer): Story triage, INVEST decomposition, living architecture plane synchronization (7-day SLA), and PR architectural reviews.
       - `node-devtest` (Node 2: Consumer): Pre-flight workspace validation, destructive git safety verification, deterministic lowest-ID task pickup (`StateManager.get_next_devtest_task`), test-driven implementation, local/CI verification, autonomous auto-merge, pull request generation, and sequential parent advancement (`_advance_sequential_subtask`).
...
│   ├── worktree.py                  # Ephemeral git worktree manager, stash protection & fallback
│   ├── ui/                          # Presentation & TUI dashboard package
│   │   ├── __init__.py              # Subpackage exports
│   │   ├── dashboard.py             # DashboardApp Textual TUI with DataTable & RichLog
│   │   └── widgets.py               # Read-only UI widgets (SDLCProgressWidget, AnomalyAlertsWidget, HarnessQuotaWidget, ConfigStatusBanner)
...
### 4. Pluggable AI Harness Adapter Pattern (`AsyncHarnessAdapter`)
...
### 5. Distributed State Machine & TTL Locking (`StateManager`)
...
- **Startup Orphan Lock Reclamation**: On daemon startup (`register_daemon`) and single-pass CLI runs, `cleanup_orphaned_running_jobs()` automatically identifies and transitions orphaned `RUNNING` locks left behind by killed or crashed processes to `FAILED`, preventing deadlock freezes.
...
### 6. Dynamic Hot Reloading Runtime (`SourceWatcher`, `_daemon_reload_watcher`, `ConfigHolder` & `_rebind_config`)
...
### 7. Real-Time Active Node Log Streaming & Seamless Offset Handoff
- **Zero-Refresh Streaming**: Real-time live execution logs stream directly into `RichLog` without requiring manual tab switches or table clicks.
- **Identity Transition Diffing**: `update_projects_table` tracks `(project_name, active_node)` pairs to detect job transitions (such as Idle $\to$ active node) even when table cursor indices remain unchanged.
- **Seamless Offset Handoff**: Upon completion of `hydrate_project_logs`, `_last_tail_file = target_file` and `_last_tail_offset = file_size` are recorded. The incremental tailer `_poll_active_log_file` seeks strictly to byte offset N (`f.seek(self._last_tail_offset)`) on the 2.0s table refresh tick, eliminating duplicate or dropped lines.
- **Disambiguated Startup Placeholders**:
  1. `"No execution logs found yet for node '{node_name}'"` when no log files exist on disk.
  2. `"⚡ Initializing {node_name} harness on Issue #{issue_id}... Awaiting output."` when an initial 0-byte active startup file is detected.
  3. Clean retirement of placeholder upon arrival of the harness's first byte.
- **Log Rotation & Truncation Recovery**: Detects file truncation (`current_size < self._last_tail_offset`) or rotation, resets state, and re-discovers active log files cleanly.
- **Direct Asyncio Main-Loop Writes**: Safely performs direct writes to `log_view.write(rich.markup.escape(line))` on the main asyncio thread, preserving `call_from_thread` marshaling exclusively for background threads.

### 8. Dependency Injection via Composition Root
Configuration is loaded once via `load_config()` at the presentation entry point (`cli.py`), which acts as the **Composition Root**. Dependencies (`config`, `project`, `state_manager`, `quota_manager`, `worktree_manager`) are instantiated and explicitly injected down the call hierarchy into use cases and widgets. Modules never rely on global mutable singletons.

### 9. Multi-Window Rolling Quota & Velocity Runway Gating Pattern (`QuotaManager` & `TokenUsageReader`)
...
### 10. Ephemeral Worktree Isolation & Fallback Pattern (`WorktreeManager`)
...
- **Process Safety & Subprocess Timeout Hardening**: All worktree and git operations invoke `run_git_command(args, cwd, timeout=30.0)`. Upon `asyncio.TimeoutError`, `_kill_process_tree(proc)` recursively terminates all child processes, calls `proc.kill()`, and awaits `proc.wait()`, returning `(-1, "", "Git command timed out after ...")` and preventing orphaned git processes from holding `.git/index.lock` on Windows.
- **Detached HEAD Upstream Synchronization**: `checkout_detached_upstream(cwd, branch="main")` synchronizes ephemeral worktrees by fetching `origin/<branch>`, detaching HEAD (`git checkout --detach origin/<branch>`), and hard resetting (`git reset --hard origin/<branch>`), completely eliminating branch collision errors (`fatal: '<branch>' is already checked out`).
...
### 11. Single-Pass Bulk PR State & CI Status Rollup Extraction Pattern (`poller.py`)
