...
""",
    ...
    result = runner.invoke(app, ["quota", "status", "--config", str(config_file)])
    assert result.exit_code == 0
...
def test_cli_run_fails_fast_on_quota_exhaustion(tmp_path: Path):
    """
    Scenario 2: One-shot run fails fast on insufficient quota.
    When target node's harness lacks required runway, pre-flight validation prints
    descriptive error detailing quota deficit and replenishment countdown, exits immediately
    with exit code 2, and no issue state is mutated.
    """
    from orchestrator.db import StateManager
