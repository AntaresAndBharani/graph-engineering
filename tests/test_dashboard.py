...
# ---------------------------------------------------------------------------
# Issue #158 BDD Scenarios: End-to-End Dashboard Log Streaming Verification
# Suite & Node-CLI Documentation Verification
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_scenario_live_streaming_without_manual_refresh_for_active_running_node(tmp_path: Path, mocker):
    """
    ...
    project_log_dir = logs_root / "biq-playbook" / "architect"
    project_log_dir.mkdir(parents=True, exist_ok=True)
    log_file = project_log_dir / "20260903_140000_architect_run.log"
    log_file.touch()
    ...
    async with app.run_test() as pilot:
        ...
        # Table update tick runs automatically (simulating 2.0s background timer)
        await app.update_projects_table()
        await pilot.pause()
        ...
        call_from_thread_spy = mocker.spy(app, "call_from_thread")
        refresh_spy = mocker.spy(app, "refresh")

        # When new log output is generated by the AI harness
        harness_line_1 = "  [biq-playbook:architect] Initializing harness context for Issue #75"
        harness_line_2 = "  [biq-playbook:architect] Loading repository architecture & standards"
        ...
        # Then the "Logs" tab must display the active log output in real time
        rendered_texts = [line.text for line in log_view.lines]
        assert harness_line_1 in rendered_texts
        assert harness_line_2 in rendered_texts

        # And the operator must see the output without pressing "r" or refreshing
        refresh_spy.assert_not_called()
        assert not any("⚡ Initializing" in t for t in rendered_texts)
...
@pytest.mark.asyncio
async def test_scenario_strict_node_isolation_across_disk_and_in_memory_buffers_dashboard(tmp_path: Path):
    """
    ...
    logs_root = tmp_path / "logs"
    devtest_log_dir = logs_root / "biq-playbook" / "devtest"
    devtest_log_dir.mkdir(parents=True, exist_ok=True)
    devtest_log = devtest_log_dir / "20260903_150000_devtest_run.log"
    devtest_log.write_text("DevTest execution output line 1\nDevTest output line 2\n", encoding="utf-8")

    # Untagged lines in memory
    ProjectLogBufferManager.add_line("Untagged daemon system message 1", project_name="biq-playbook")
    ProjectLogBufferManager.add_line("Untagged daemon system message 2", project_name="biq-playbook", node_name=None)
    # DevTest lines in memory
    ProjectLogBufferManager.add_line("DevTest in-memory trace", project_name="biq-playbook", node_name="devtest")
    ...
    async with app.run_test() as pilot:
        ...
        # When get_project_logs is invoked with node_name='architect' (via hydrate_project_logs)
        await app.hydrate_project_logs("biq-playbook", node_name="architect")
        await pilot.pause()

        # Then it must NOT return devtest logs or untagged in-memory lines
        rendered_texts = [line.text for line in log_view.lines]
        assert not any("DevTest execution output" in t for t in rendered_texts)
        assert not any("Untagged daemon system message" in t for t in rendered_texts)
        assert not any("DevTest in-memory trace" in t for t in rendered_texts)

        # And the log pane must display "No execution logs found yet for node 'architect'"
        assert rendered_texts == ["No execution logs found yet for node 'architect'"]
        assert app._placeholder_active is True


def test_scenario_zero_refresh_end_to_end_observability_documentation():
    """
    Scenario: Zero-refresh end-to-end observability documentation
      Given the TUI dashboard supports real-time streaming and offset handoff
      When an operator inspects "docs/node-cli.md"
      Then it must document the automatic node scope inference, disambiguated placeholders,
      and zero-refresh streaming behavior.
    """
    ...
    # Document automatic node scope inference and identity transition
    assert "Automatic Node Scope Inference" in content or "node scope inference" in content.lower()
    assert "(project_name, active_node)" in content or "identity diffing" in content.lower()

    # Document disambiguated placeholders
    assert "No execution logs found yet for node" in content
    assert "⚡ Initializing" in content
    assert "Clean Retirement" in content or "clean retirement" in content.lower()

    # Document zero-refresh streaming behavior
    assert "Zero-Refresh" in content or "zero-refresh" in content.lower()
    assert "Offset Handoff" in content or "offset handoff" in content.lower()
    assert "call_from_thread" in content
    assert "log_view.write" in content

    # Document strict node scope isolation and log rotation recovery
    assert "Strict Node Scope Isolation" in content or "strict node scope isolation" in content.lower()
    assert "Log Rotation Recovery" in content or "log rotation recovery" in content.lower()


@pytest.mark.asyncio
async def test_scenario_end_to_end_full_dashboard_lifecycle_log_streaming_suite(tmp_path: Path, mocker):
    """
    Comprehensive End-to-End BDD Integration Test:
    Executes full multi-node lifecycle:
    1. Idle State: project 'biq-playbook' displayed, idle title.
    2. Architect Execution on Issue #75:
       - Identity transition auto-detected.
       - 0-byte startup placeholder displayed.
       - Live harness streams output -> placeholder cleanly retired without 'r' keypress.
       - Disk writes seamlessly tailed via offset handoff without duplication.
    3. Transition from Architect to DevTest on Issue #76:
       - Identity transition diffing triggers.
       - DevTest 0-byte placeholder displayed.
       - Live output arrives and streams directly.
    4. Log Rotation Recovery:
       - Actively tailed DevTest file is rotated/unlinked.
       - Incremental poll catches FileNotFoundError, resets tracked state.
       - Fresh rotated log file created and cleanly re-discovered.
    5. Return to Idle State:
       - Lock released -> project returns to Idle cleanly.
       - Log view title and state reset to Idle.
    """
    ProjectLogBufferManager.reset()

    logs_root = tmp_path / "logs"
    arch_dir = logs_root / "biq-playbook" / "architect"
    devtest_dir = logs_root / "biq-playbook" / "devtest"
    arch_dir.mkdir(parents=True, exist_ok=True)
    devtest_dir.mkdir(parents=True, exist_ok=True)

    arch_log_file = arch_dir / "20260903_160000_architect.log"
    arch_log_file.touch()
    ...
    async with app.run_test() as pilot:
        ...
        # Step 2: Architect starts executing on Issue #75
        await state_manager.acquire_lock(issue_id=75, repo="BasketIQ/biq-playbook", node_type="architect")
        await app.update_projects_table()
        await pilot.pause()
        ...
        app._handle_harness_stream_line(
            project_name="biq-playbook",
            node_name="architect",
            line="Architect: Analyzing Issue #75 Gherkin scenarios",
        )
        await pilot.pause()
        ...
        # Step 3: Architect finishes, DevTest starts on Issue #76
        await state_manager.release_lock(issue_id=75, repo="BasketIQ/biq-playbook", node_type="architect")
        await state_manager.acquire_lock(issue_id=76, repo="BasketIQ/biq-playbook", node_type="devtest")

        devtest_log_file = devtest_dir / "20260903_161000_devtest.log"
        devtest_log_file.touch()
        ...
        app._handle_harness_stream_line(
            project_name="biq-playbook",
            node_name="devtest",
            line="DevTest: Running pytest unit verification suite",
        )
        await pilot.pause()
        ...
        # Fresh rotated log arrives
        devtest_rotated = devtest_dir / "20260903_162000_devtest.log"
        devtest_rotated.write_text("DevTest rotated: all 50 tests passed\n", encoding="utf-8")
